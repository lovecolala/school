...
%% Data pair
input = CCPP(1:1000, 1:end-1)';
target = CCPP(1:1000, end)';

N_h = 4;
num_data = length(CCPP);
...
eval(['C = allcomb(', t, ');']);
%% Block Selection 
% this 'C' is new Construct Matrix
C = BlockSelection(C, input(:, 1:training), h);
N_Rule = size(C, 1);
...
xi1 = rand; xi2 = rand;
%% training
iteration = 50;     % iterative of training
