...
for i = 1:size(C, 1)            % Rule
        ...
        beta = FirStrg(input(:, 1:training), CenterSigma);
...
num = 50;
for i = 1:num
        particle(i).position = IFpart_para*randn;
...
for i = 1:T_round
        for q = 1:num         %particle
                ...
                for k = 1:N_Rule             %Rule
                        ...
                        particle(q).beta(k, :) = FirStrg(input(:, 1:training), CenterSigma);
                end
                % Normalization
                particle(q).beta = particle(q).beta ./ sum(particle(q).beta, 1);     % get real
                particle(q).beta = [repmat(particle(q).beta(1, :), 3, 1); repmat(particle(q).beta(2, :), 3, 1); repmat(particle(q).beta(3, :), 3, 1)];
...
for i = 1:N_Rule
        ...
        beta(k, :) = FirStrg(input, CenterSigma);
end
% Normalization
% beta = real(beta);
beta = beta ./ sum(beta, 1);     % get real
beta = [repmat(beta(1, :), 3, 1); repmat(beta(2, :), 3, 1); repmat(beta(3, :), 3, 1)];
% beta = reshape(repmat(beta', N_h+1, 1), 1, []);
theta = RLSE(input_para.*beta, target);
