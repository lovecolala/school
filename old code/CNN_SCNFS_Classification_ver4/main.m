...
target(tt) = 1;
% ---------------------------
target_complex = target(:, 1:2:end) + target(:, 2:2:end) .* sqrt(-1);
...
[result_RMSE, output_complex] = model(Leader_pos, opts, 0);     % 不需要再做RLSE
...
for i = 1:nTarget*nClass/2
        output(:, 2*i-1) = real(output_complex(:, i));
        output(:, 2*i) = imag(output_complex(:, i));
end
