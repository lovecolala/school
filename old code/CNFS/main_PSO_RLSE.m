...
% Constructor
index = repmat(class_index, CM_length_new, 1);
temp_start = 1;
temp_end = 0;
for i = 1:num_inputs
...
for i = 1:num_Particle
    ...
    particle(i).pos = randn(dimensionSize); 
    particle(i).vel = zeros(dimensionSize);
    costFunction = costFunction_PSO_RLSE(data_pairs(1:num_training_data,:), particle(i).pos, conMatrix_new, index, num_if_part, num_then_part);
    particle(i).cost = costFunction.cost;
    ...
    for i = 1:num_Particle
        ...
        if particle(i).cost < particle(i).bestCost
            ...
            if particle(i).bestCost < globalBest.cost
                globalBest.pos = particle(i).pos;
                globalBest.cost = particle(i).cost;
                globalBest.thenPos = costFunction.pos;
            end
        end  
    end
...
hold on;
output = real(outputFunction_all_PSO(data_pairs, [globalBest.pos globalBest.thenPos], conMatrix_new, index, num_if_part));
