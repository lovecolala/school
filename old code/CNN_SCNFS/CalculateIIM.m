...
for i = 1:nTarget
        t = delta(:, i);
        feature_table = [feature_table, t(index_feature)];
        target = [target, t(index_target)];
end
feature_table = [feature_table, target];

Entropy =@(pd, phi, dx) sum(pd .* log10( phi./pd )) .* dx;
Entropy_condition =@(pdx, pdyx, phi, dx, dyx) sum(pdx .* sum(pdyx .* log10( phi ./ pdyx ))) .* dx .* dyx;
%% Influence Information
nFeature_table = size(feature_table, 2);
IIM = zeros(nFeature_table);
for i = 1:nFeature_table
        %% f1+和f1-
        % Feature為正及為負的真實資料
