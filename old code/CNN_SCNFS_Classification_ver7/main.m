...
if strcmp(target_type, 'real')
        opts.target = target.real;
elseif strcmp(target_type, 'complex')
...
if strcmp(target_type, 'complex')
        [result_RMSE, output.complex] = model(Leader_pos, opts, 0);     % 不需要再做RLSE
elseif strcmp(target_type, 'real')
        [result_RMSE, output.real] = model(Leader_pos, opts, 0);     % 不需要再做RLSE
end
...
for i = 1:nTarget
        ...
        output.classification(:, i) = nClass - b;
