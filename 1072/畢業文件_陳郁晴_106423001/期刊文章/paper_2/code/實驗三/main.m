...
%%
for nFeatureDate = [10, 30, 50]
        for isworse = 0:1
                nStock = 4;
                nTarget = 4;
                stockname = {'KOSPI', 'Nikkei 225', 'HSI', 'TAIEX'};
                load(['dataset_', num2str(nFeatureDate)]);
                ...
                load(['IIM_', num2str(nFeatureDate)]);
                
                fs.FP = FeatureSelection(fs.data_table, IIM, nTarget);
                if isworse == 1
                        nCP = 16*nFeatureDate;
                        fs.FP = setdiff(1:nCP, fs.FP);
                end
                fs.FP = sort(fs.FP);
                nFP = length(fs.FP);
                ...
                %% RLSE
                nPara = (1+nDim)*nThen*nOutput;
                algoindex.theta = reshape((1:nPara), (1+nDim)*nThen, nOutput) + accu_Para;
                accu_Para = accu_Para + nPara;
                position_rlse = randn(1, (1+nDim)*nThen*nOutput);
                ...
                for e = 1:nExp
                        ...
                        for i = 1:nTarget
                                ...
                                text(date(nTraining/2 + nFeatureDate), y_max*49/50, {'Training'; 'phase'}, ...
                                ...
                                line(date([nTraining, nTraining]+nFeatureDate), get(gca, 'ylim'), 'linestyle', '--', 'linewidth', 1.5, 'color', 'k');
                                text(date(nTraining/2 + nFeatureDate), y_max*9/10, {'Training'; 'phase'}, ...
                                        'HorizontalAlignment', 'center', 'color', 'k');
                ...
                savefig(F, ['result_', num2str(nFeatureDate), '_', num2str(isworse)]);
                [~, min_idx] = min(result_RMSE);
                close(1:(nTarget*2+1)*(min_idx-1));
                close((nTarget*2+1)*min_idx+1:(nTarget*2+1)*nExp);
                F = findall(0, 'type', 'figure');
                savefig(F, ['result_', num2str(nFeatureDate), '_', num2str(isworse), '_thebest']);
                close all;
                save(['result_', num2str(nFeatureDate), '_', num2str(isworse)]);
        end
end