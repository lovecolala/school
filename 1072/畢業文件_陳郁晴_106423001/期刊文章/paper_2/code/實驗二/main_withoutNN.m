...
nFP = 4;
fs.FP = FeatureSelection(fs.data_table, IIM, nTarget, nFP);
fs.FP = sort(fs.FP);
...
h = Subclustering(fs.output(1:nTraining, :)', Ra);
t = []; comma = [];
for i = 1:nFP
...
eval(['C = allcomb(', t, ');']);
C = BlockSelection(C, fs.output(1:nTraining, :)', h, nTarget, 10);
[nRule, nDim] = size(C);
